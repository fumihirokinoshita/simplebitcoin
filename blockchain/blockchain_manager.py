...
class BlockchainManager:
    ...
    def remove_useless_transaction(self, transaction_pool):
        """
        与えられた Transactionのリストの中ですでに自分が管理する
        ブロックチェーン内に含まれたTransactionがある場合、
        それを削除したものを返却する
        """
    ...
    def resolve_conflicts(self, chain):
        ...
        # 有効性検証自体はrenew_my_blockchainで実施
        ...
        # 自分のチェーンの中でだけ処理済みになっているTransactionを救出する。
        # 現在のチェーンに含まれていないブロックを全て取り出す。
        # 時系列を考えての有効無効判定などはしないかなり簡易な処理。
